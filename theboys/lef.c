...
{
  struct nodo_lef_t *a, *auxiliar, *anteriorAuxiliar ;

	a = malloc (sizeof (struct nodo_lef_t)) ;
  ...
  {
    auxiliar = l->primeiro ;
    if (auxiliar->evento->tempo > e->tempo)
    {
        l->primeiro = a ;
    ...
    else 
    {
	    while (auxiliar->prox != NULL) 
	    {
        	...
        	if (auxiliar->evento->tempo > e->tempo || (auxiliar->evento->tempo == e->tempo && auxiliar->evento->tipo < e->tipo))
       		{
         		 a->prox = auxiliar ;
         		 anteriorAuxiliar->prox = a ;
           		 return 1;
        	}
      }
    }
